...
namespace Reservation
{
    public class PaieForm : Form
    {
        ...
        private void MainPanel()
        {
            ...
            montant = CreateFormControl("Montant", ref y, controlHeight, sideCenter);
            date = CreateFormControlDate("Date de départ", ref y, controlHeight, sideCenter);
        ...
        {
            dgPaie.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 10, FontStyle.Bold);
            dgPaie.ColumnHeadersHeight = 50;
            dgPaie.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(248, 248, 248);
            dgPaie.RowTemplate.Height = 40;
        }
        private void FormatDataGrid()
        {
            if (dgPaie.Columns.Count > 0)
            {
                dgPaie.Columns["idpaie"].HeaderText = "ID";
                dgPaie.Columns["idreservation"].HeaderText = "Reservation";
                dgPaie.Columns["montant"].HeaderText = "Montant";
                dgPaie.Columns["datepaie"].HeaderText = "Date de Paiement";
            }
        }
        ...
        private void RemplirForm()
        {
            ...
            {
                string id = dgPaie.CurrentRow.Cells[0].Value?.ToString();
                string idres = dgPaie.CurrentRow.Cells[1].Value?.ToString();
                string Montant = dgPaie.CurrentRow.Cells[2].Value?.ToString();
                string dateP = dgPaie.CurrentRow.Cells[3].Value?.ToString();

                idtxt.Text = id;
                cbRes.SelectedValue = idres;
                montant.Text = Montant;
                if (DateTime.TryParse(dateP, out DateTime date1))
                {
                    date.Value = date1;
                }
            }
        ...
        {
            string res = "";
            if (string.IsNullOrEmpty(idtxt.Text.Trim()) || string.IsNullOrEmpty(montant.Text.Trim()))
            {
        ...
        private void Ajouter()
        {
            if (VerifierChamps())
            {
                try
                {
                    DateTime dP = date.Value;
                    int M = int.Parse(montant.Text.Trim());
                    var paie = new Paiement(idtxt.Text, cbRes.SelectedValue.ToString(), M, dP);

                    service.Ajouter(paie);
                    ChargerPaie();
                    ViderChamps();
        ...
        {
            dgPaie.DataSource = service.ObtenirTous();
            FormatDataGrid();
        }
        ...
        private void Rechercher()
        {
            ...
            dv.RowFilter = $"idpaie LIKE '%{texte}%' OR idreservation LIKE '%{texte}%'";

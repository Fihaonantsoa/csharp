...
{
    public class BilletForm : Form
    {
        private ResService ResServe;
        private PassagerService PasServe;
        private VolService Volserve;
        private BilletService service;
        private ComboBox idPass, idVol, idRes;
        private TextBox idtxt, recherche, classe, siege, prix, etat;
        private Button btnAjouter, btnModifier, btnSupprimer, btnActualiser;
        private DataGridView dgBillet;
        ...
        public BilletForm()
        {
            ...
            PasServe = new PassagerService();
            Volserve = new VolService();
            ChargerBillet();
            ChargerPassager();
            ChargerResevation();
            ChargerVol();
        ...
        private void MainPanel()
        {
            ...
            idPass = CreateFormControlCombo("Passager", ref y, controlHeight, sideCenter);
            idVol = CreateFormControlCombo("Vol", ref y, controlHeight, sideCenter);
            idRes = CreateFormControlCombo("Reservation", ref y, controlHeight, sideCenter);
            classe = CreateFormControl("Classe", ref y, controlHeight, sideCenter);
            siege = CreateFormControl("N° Siege", ref y, controlHeight, sideCenter);
            prix = CreateFormControl("Prix", ref y, controlHeight, sideCenter);
            etat = CreateFormControl("Etat", ref y, controlHeight, sideCenter);
            ...
            dgBillet.SelectionChanged += (s, e) => dgVol_selected(s, e);
            StyleDataGrid();

            Dgpanel.Controls.Add(dgBillet);
        ...
        {
            dgBillet.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 10, FontStyle.Bold);
            dgBillet.ColumnHeadersHeight = 50;
            dgBillet.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(248, 248, 248);
            dgBillet.RowTemplate.Height = 40;
        }
        private void FormatDataGrid()
        {
            if (dgBillet.Columns.Count > 0)
            {
                dgBillet.Columns["idbillet"].HeaderText = "ID";
                dgBillet.Columns["idvoyageur"].HeaderText = "Passager";
                dgBillet.Columns["idvol"].HeaderText = "Vol";
                dgBillet.Columns["idreservation"].HeaderText = "Réservtion";
                dgBillet.Columns["classe"].HeaderText = "Classe";
                dgBillet.Columns["siege"].HeaderText = "N° Siege";
                dgBillet.Columns["prix"].HeaderText = "Prix";
                dgBillet.Columns["etat"].HeaderText = "Etat";
            }
        }
        ...
        {
            if (dgBillet.CurrentRow != null)
            {
                string id = dgBillet.CurrentRow.Cells[0].Value?.ToString();
                string IDv = dgBillet.CurrentRow.Cells[1].Value?.ToString();
                string IDvol = dgBillet.CurrentRow.Cells[2].Value?.ToString();
                string IDres = dgBillet.CurrentRow.Cells[3].Value?.ToString();
                string Classe = dgBillet.CurrentRow.Cells[4].Value?.ToString();
                string Siege = dgBillet.CurrentRow.Cells[5].Value?.ToString();
                string Prix = dgBillet.CurrentRow.Cells[6].Value?.ToString();
                string Etat = dgBillet.CurrentRow.Cells[7].Value?.ToString();

                idtxt.Text = id;
                idPass.SelectedValue = IDv;
                idVol.SelectedValue = IDvol;
        ...
        {
            string res = "";
            if (string.IsNullOrEmpty(idtxt.Text.Trim()) || string.IsNullOrEmpty(classe.Text.Trim())
                || string.IsNullOrEmpty(siege.Text.Trim()) || string.IsNullOrEmpty(prix.Text.Trim())
                || string.IsNullOrEmpty(etat.Text.Trim()))
            {
        ...
        private void Ajouter()
        {
            if (VerifierChamps())
            {
                try
                {
                    int S = int.Parse(siege.Text.Trim());
                    int P = int.Parse(prix.Text.Trim());
                    var billet = new Billet(idtxt.Text, idPass.SelectedValue.ToString(), idVol.SelectedValue.ToString(), idRes.SelectedValue.ToString(), classe.Text, S, P, etat.Text);

                    service.Ajouter(billet);
                    ChargerBillet();
                    ViderChamps();
        ...
        {
            dgBillet.DataSource = service.ObtenirTous();
            FormatDataGrid();
        }
        ...
        private void Rechercher()
        {
            ...
            dv.RowFilter = $"idbillet LIKE '%{texte}%' OR idvoyageur LIKE '%{texte}%' OR idvol LIKE '%{texte}%' OR idreservation LIKE '%{texte}%' OR classe LIKE '%{texte}%' OR etat LIKE '%{texte}%'";
            dgBillet.DataSource = dv;
        }
        private void ChargerResevation()
        {
            ...
            idRes.Items.Clear();
            idRes.DataSource = dt;
            idRes.DisplayMember = "idreserve";
            idRes.ValueMember = "idreserve";
        }
        private void ChargerPassager()
        {
            DataTable dt = PasServe.ObtenirTous();
            idPass.Items.Clear();
            idPass.DataSource = dt;
            idPass.DisplayMember = "id";
            idPass.ValueMember = "id";
        }
        private void ChargerVol()
        {
            DataTable dt = Volserve.ObtenirTous();
            idVol.Items.Clear();
            idVol.DataSource = dt;
            idVol.DisplayMember = "idvol";
            idVol.ValueMember = "idvol";
        }
        private void ViderChamps()
        {
            idtxt.Text = "";
            idPass.SelectedIndex = -1;
            idVol.SelectedIndex = -1;
            idRes.SelectedIndex = -1;
            classe.Text = "";
            siege.Text = "";
            prix.Text = "";
            etat.Text = "";
